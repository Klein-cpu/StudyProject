...
void UBaseGameInstance::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(UBaseGameInstance, ServerName);
	DOREPLIFETIME(UBaseGameInstance, MaxPlayers);
}
...
void UBaseGameInstance::LaunchLobby(uint32 MaxPlayers_In, FName ServerName_In, bool bIsLAN)
{
...
void UBaseGameInstance::FindAMatch(bool bIsLAN)
{
	FindSessions(GetPrimaryPlayerUniqueId(), bIsLAN, true);
}

void UBaseGameInstance::JoinOnlineGame()
{
...
bool UBaseGameInstance::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN,
...
void UBaseGameInstance::FindSessions(TSharedPtr<const FUniqueNetId> UserId, bool bIsLAN, bool bIsPresence)
{
...
bool UBaseGameInstance::JoinFoundOnlineSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName,
...
void UBaseGameInstance::OnNetworkFailure(UWorld* World, UNetDriver* NetDriver, ENetworkFailure::Type FailureType,
	const FString& ErrorMessage)
{
	DisplayNetworkErrorMessage(ErrorMessage);
}

void UBaseGameInstance::OnTravelFailure(UWorld* World, ENetworkFailure::Type FailureType, const FString& ErrorMessage)
{
	DisplayNetworkErrorMessage(ErrorMessage);
}

void UBaseGameInstance::DisplayNetworkErrorMessage(FString ErrorMessage)
{
